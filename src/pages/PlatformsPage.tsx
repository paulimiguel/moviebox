    ...
    <main className="min-h-screen bg-canvas">
      ...
      {deleting && <div className="fixed inset-0 z-[70] grid place-items-center bg-ink/55 p-4" role="dialog" aria-modal="true" aria-labelledby="delete-platform-title">
        <div className="movie-detail-modal w-full max-w-md overflow-hidden rounded-md bg-canvas shadow-xl">
          <header className="flex min-h-16 items-center border-b border-slate-200 bg-white px-5"><h2 id="delete-platform-title" className="text-lg font-semibold text-ink">Eliminar plataforma</h2></header>
          <div className="p-5">
          ...
          <footer className="flex justify-end gap-2 border-t border-slate-200 bg-white px-5 py-4"><button type="button" onClick={() => setDeleting(null)} className="secondary-button">Cancelar</button><button type="button" onClick={() => deletePlatform.mutate(deleting)} disabled={deletePlatform.isPending} className="primary-button bg-red-600 hover:bg-red-700">{deletePlatform.isPending && <Loader2 className="h-4 w-4 animate-spin" />}Eliminar</button></footer>
        </div>
      </div>}
    </main>
  );
