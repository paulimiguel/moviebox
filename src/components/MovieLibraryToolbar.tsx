        ...
        <div className="min-w-0 self-center">
          ...
          {!props.hideActiveFilters && (selectedGenreNames.length > 0 || selectedPlatformNames.length > 0 || props.years.length > 0 || props.type !== 'all' || props.watched !== 'all' || props.favorite !== 'all' || props.watchlist !== 'all' || props.emptyFieldsCount > 0) && (
            <div className="mt-2 flex flex-wrap gap-1">
              ...
              {props.watched !== 'all' && <button type="button" onClick={() => props.onWatchedChange('all')} className="inline-flex items-center gap-1 rounded bg-mist px-1.5 py-0.5 text-[11px] font-medium text-slate-600" title={`Quitar filtro ${props.watched === 'watched' ? 'Watch' : 'Unwatched'}`}>{props.watched === 'watched' ? 'Watch' : 'Unwatched'}<X className="h-3 w-3" /></button>}
