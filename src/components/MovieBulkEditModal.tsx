...
export const MovieBulkEditModal = ({ movies, genres, platforms, collections, onClose, onSaved }: {
  ...
  const [instagramRecommendation, setInstagramRecommendation] = useState<'' | 'yes' | 'no'>('');
  ...
  const mutation = useMutation({
    mutationFn: () => Promise.all(movies.map((movie) => {
      ...
      if (instagramRecommendation) input.instagramRecommendation = instagramRecommendation === 'yes';
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-ink/55 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="bulk-edit-title">
      <div className="movie-detail-modal w-full rounded-t-md bg-canvas shadow-xl sm:max-w-xl sm:rounded-md">
        ...
        <div className="grid gap-4 bg-canvas p-4 sm:grid-cols-2 sm:p-6">
          ...
          <label><span className="field-label">Recomendación de Instagram</span><select className="control w-full" value={instagramRecommendation} onChange={(event) => setInstagramRecommendation(event.target.value as '' | 'yes' | 'no')}><option value="">No cambiar</option><option value="yes">Sí</option><option value="no">No</option></select></label>
          <div>
            <span className="field-label">Estado</span>
            <div className="grid grid-cols-3 gap-2">
              <button type="button" aria-pressed={markWatched} onClick={() => setMarkWatched((current) => !current)} className={`moviebox-translucent-action inline-flex h-10 min-w-0 items-center justify-center gap-1.5 rounded-md border px-2 text-sm font-semibold transition-colors ${markWatched ? 'border-coral bg-coral text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}><Eye className="h-4 w-4 shrink-0" />Vista</button>
              <button type="button" aria-pressed={markFavorite} onClick={() => setMarkFavorite((current) => !current)} className={`moviebox-translucent-action inline-flex h-10 min-w-0 items-center justify-center gap-1.5 rounded-md border px-2 text-sm font-semibold transition-colors ${markFavorite ? 'border-coral bg-coral text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}><Heart className={`h-4 w-4 shrink-0 ${markFavorite ? 'fill-current' : ''}`} />Like</button>
              <button type="button" aria-pressed={markWatchlist} onClick={() => setMarkWatchlist((current) => !current)} className={`moviebox-translucent-action inline-flex h-10 min-w-0 items-center justify-center gap-1.5 rounded-md border px-2 text-sm font-semibold transition-colors ${markWatchlist ? 'border-aqua bg-aqua text-white' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}><Bookmark className={`h-4 w-4 shrink-0 ${markWatchlist ? 'fill-current' : ''}`} />Watchlist</button>
            </div>
          </div>
